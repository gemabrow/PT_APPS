(defun c:drawsup ()
   ...
   (setq continue "Y")
   (while (or (= (strcase continue) "Y") (= continue ""))
      ...
      ; DETERMINE DIM-MODE
      (setq mode "aligned")
      (cond ((= (cadr (nth 0 epl1)) (cadr (nth 1 epl1)))
               (setq mode "hor")
            )
      ...
      ;(setq lp (getreal "Enter next lowpoint cgs in inches if required, else return: "))
      ;(if (= lp nil)
      ;   (progn
      ;      (initget "Y y N n")
      ...
      ;      (setq p3 (getpoint "Select both endpoints of support at next highpoint: "))
      ;      (setq p4 (getpoint))
      ;      (terpri)
      ;      (if (= (getvar "orthomode") 1) ;MAKE P4 ORTHO W/ RESPECT TO P3 IF ORTHOMODE IS ON		
      ;         (if (< (abs (- (car p3) (car p4))) (abs (- (cadr p3 ) (cadr p4))))
      ...
      ;         (setq sup_exist 'Y)
      ;      )   
      ;      (setq hp2 (getreal "Enter highpoint cgs in inches: "))   
      ;   );END PROGN
      ;  (progn
      ;      (initget "Y y N n")
